...
export function About() {
  ...
  useEffect(() => {
    const loadPageData = async () => {
      try {
        const data = await staticPagesService.getStaticPage('about');
        setPageData(data);
      } catch (error) {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-12">
        ...
        <p className="text-xl text-gray-600 max-w-3xl mx-auto">
          NYC's premier Jewish rental platform, connecting tenants, landlords, and agents 
          in a trusted community-focused environment.
        </p>
      </div>
